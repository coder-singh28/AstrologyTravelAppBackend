...
class app_utils:
    ...
    def verify_session(session_token: str, user_id: str, database_utils, logger) -> bool:
        ...
        query = f"SELECT * FROM session_details WHERE session_token = %s AND user_id = '{user_id}' AND expires_at > NOW()"
        try:
            result = database_utils.performeSelectStatement(query, (session_token), logger)
